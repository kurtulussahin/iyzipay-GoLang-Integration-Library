...
func ExampleRetrieveBkmResult() {
	options := iyzipay.Options{}
	options = builder.CreateSampleOptions()

	request := iyzipay.RetrieveBkmRequest{
		Locale:         "tr",
		ConversationId: "123456789",
		Token:          "token",
	}

	rawResponse := iyzipay.Bkm{}.Retrieve(request, options)
	...
	//This test needs manual bkm redirection on sandbox environment. So it does not contain any assertions.
}
