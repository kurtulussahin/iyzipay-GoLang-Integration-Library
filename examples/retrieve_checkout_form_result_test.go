...
func ExampleRetrieveCheckoutFormResult() {
	options := iyzipay.Options{}
	options = builder.CreateSampleOptions()

	request := iyzipay.RetrieveCheckoutFormRequest{
		Locale:         "tr",
		ConversationId: "123456789",
		Token:          "token",
	}

	rawResponse := iyzipay.CheckoutForm{}.Retrieve(request, options)
	...
	//This test needs manual checkout form redirection on sandbox environment. So it does not contain any assertions.
}
